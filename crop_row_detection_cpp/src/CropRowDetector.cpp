...
double CropRowDetector::CrossCorrelation(int row_number, old_tuple_type template_var_param,
    ...
    int kEnd = (int) floor(
            (((int) image_width / 2 - phase) + halfa) / period);   // offset ultima onda prima della fine dell'immagine
...
CropRowDetector::find_best_parameters(std::vector<std::map<old_tuple_type, double>> &energy_map) {
    ...
    phase_type phase;
    energy_type Dnrm;
    const energy_type maxz = (const energy_type) ((1.0 + (m_lambda_c + m_lambda_d) * m_nd * m_nd) / (m_lambda_c * m_lambda_d));
    int parabola_idx;
        ...
        if (row_number < image_height - 1) {
            ...
            for (period_idx = 0; period_idx < m_nd; period_idx++, dataset_period_ptr += m_nc) {
                ...
                for (phase = first_phase + (phase_type) 1; phase < -first_phase; phase++, dataset_tuple_ptr++) {
                    ...
                    while (true) {
                        intersection_point = numerator - dataset_period_ptr[parabola_center[parabola_idx] + m_nc / 2].tuple_value;
                        intersection_point -= m_lambda_c * parabola_center[parabola_idx] * parabola_center[parabola_idx];
                ...
                for (period_idx = 1; period_idx < m_nd; period_idx++, dataset_tuple_ptr += m_nc) {
                    ...
                    while (true) {
                        intersection_point = (numerator -
                                              (dataset_period_ptr[m_nc * parabola_center[parabola_idx]].tuple_value +
...
int CropRowDetector::period_min(const phase_type phase, period_type* periods) {
